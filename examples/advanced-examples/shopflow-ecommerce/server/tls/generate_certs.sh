...
# Generate TLS certificates for ShopFlow E-commerce gRPC testing
set -e

echo "🔐 Generating TLS certificates for ShopFlow E-commerce gRPC testing..."

# Create certificate authority (CA)
echo "1. Creating Certificate Authority (CA)..."
openssl genrsa -out ca-key.pem 4096
openssl req -new -x509 -key ca-key.pem -sha256 -subj "/C=US/ST=CA/O=ShopFlow E-commerce Testify/CN=Test CA" -days 365 -out ca-cert.pem
...
openssl req -new -key server-key.pem -out server.csr -subj "/C=US/ST=CA/O=ShopFlow E-commerce Testify/CN=localhost"
...
openssl req -new -key client-key.pem -out client.csr -subj "/C=US/ST=CA/O=ShopFlow E-commerce Testify Client/CN=grpctestify-client"
