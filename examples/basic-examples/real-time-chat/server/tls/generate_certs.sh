...
# Generate TLS certificates for gRPC chat server testing
set -e

echo "🔐 Generating TLS certificates for Chat gRPC testing..."

# Create certificate authority (CA)
echo "1. Creating Certificate Authority (CA)..."
openssl genrsa -out ca-key.pem 4096
openssl req -new -x509 -key ca-key.pem -sha256 -subj "/C=US/ST=CA/O=gRPC Chat Testify/CN=Test CA" -days 365 -out ca-cert.pem
...
openssl req -new -key server-key.pem -out server.csr -subj "/C=US/ST=CA/O=gRPC Chat Testify/CN=localhost"
...
openssl req -new -key client-key.pem -out client.csr -subj "/C=US/ST=CA/O=gRPC Chat Testify Client/CN=grpctestify-client"
